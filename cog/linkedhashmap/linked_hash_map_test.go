...
func TestLinkedHashMapInterface(t *testing.T) {
	var _ cog.Map[int, int] = NewLinkedHashMap[int, int]()
	var _ cog.IterableMap[int, int] = NewLinkedHashMap[int, int]()
}

func testToLinkedHashMap(m map[int]any) *LinkedHashMap[int, any] {
	lhm := &LinkedHashMap[int, any]{}
	for k, v := range m {
		lhm.Set(k, v)
	}
	return lhm
...
func TestLinkedHashMapContainsAny(t *testing.T) {
	cs := []struct {
		a map[int]any
		b []int
		w bool
	...
	for _, c := range cs {
		if got := testToLinkedHashMap(c.a).ContainsAny(c.b...); got != c.w {
			t.Errorf("ContainsAny(%v, %v) = %v, want %v", c.a, c.b, got, c.w)
		}
	}
}

func TestLinkedHashMapContainsAll(t *testing.T) {
	cs := []struct {
		a map[int]any
		b []int
		w bool
	...
	for _, c := range cs {
		if got := testToLinkedHashMap(c.a).ContainsAll(c.b...); got != c.w {
...
func TestLinkedHashMapUpdatingDoesntChangePairsOrder(t *testing.T) {
	lm := NewLinkedHashMap(cog.KV("foo", "bar"), cog.KV("12", "28"), cog.KV("78", "100"), cog.KV("bar", "baz"))
...
func TestLinkedHashMapTemplateRange(t *testing.T) {
	lm := NewLinkedHashMap(cog.KV("z", "Z"), cog.KV("a", "A"))
...
func TestLinkedHashMapString(t *testing.T) {
	w := `{"1":1,"3":3,"2":2}`
	a := NewLinkedHashMap(cog.KV("1", 1), cog.KV("3", 3), cog.KV("2", 2)).String()
	if w != a {
...
func TestLinkedHashMapPut(t *testing.T) {
	...
	if av, ev := m.Entries(), []cog.P[int, string]{
		...
		cog.KV(4, "d"),
		cog.KV(1, "a"),
		cog.KV(2, "b")}; !testLinkedHashMapSameEntries(av, ev) {
		t.Errorf("Got %v expected %v", av, ev)
	}
...
func testLinkedHashMapSameEntries[K comparable, V comparable](a []cog.P[K, V], b []cog.P[K, V]) bool {
	// If one is nil, the other must also be nil.
	if (a == nil) != (b == nil) {
		return false
	}
...
func assertLinkedHashMapIteratorRemove(t *testing.T, i int, it cog.Iterator2[int, int], w *LinkedHashMap[int, int]) int {
	it.Remove()
