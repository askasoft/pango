...
func (rb *RingBuffer) RemoveIter(it Iterator) {
	for it.Next() {
		rb.Remove(it.Value())
	}
}
...
func (rb *RingBuffer) RetainFunc(f func(T) bool) {
	if rb.IsEmpty() {
		return
	}
...
func (rb *RingBuffer) Each(f func(int, T) bool) {
	if rb.head <= rb.tail {
		n := 0
...
func (rb *RingBuffer) ReverseEach(f func(int, T) bool) {
	if rb.head <= rb.tail {
		n := len(rb.data) - 1
