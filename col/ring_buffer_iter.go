...
func (it *ringBufferIterator) Prev() bool {
	if it.rb.IsEmpty() {
		return false
	}
...
// Modifies the state of the iterator.
func (it *ringBufferIterator) Next() bool {
	if it.rb.IsEmpty() {
		return false
	}
...
func (it *ringBufferIterator) Value() T {
...
func (it *ringBufferIterator) SetValue(v T) {
	if it.index >= 0 && it.index < it.rb.len {
...
func (it *ringBufferIterator) Remove() {
	if it.index < 0 {
		return
	}

	it.rb.DeleteAt(it.index)
	it.start = it.index
	it.index = -1
}
...
func (it *ringBufferIterator) Reset() {
	it.start = -1
	it.index = -1
}
