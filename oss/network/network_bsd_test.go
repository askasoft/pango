...
func TestGetNetworksStats(t *testing.T) {
	networks, err := GetNetworksStats()
	if err != nil {
		t.Fatalf("error should be nil but got: %v", err)
	}

	for _, network := range networks {
		if network.Name == "en0" && network.Received <= 0 {
...
func TestCollectNetworksStats(t *testing.T) {
	got, err := collectNetworksStats(strings.NewReader(
...
func TestCollectNetworksStats2(t *testing.T) {
	got, err := collectNetworksStats(strings.NewReader(
