...
func TestParseInLocation(t *testing.T) {
	cs := []struct {
		...
		{"2020-2-3 06:07:08", "2020-02-03T06:07:08", nil},
		{"2020-02-03 06:07:08", "2020-02-03T06:07:08", nil},
		{"2020-12-13 06:07:08", "2020-12-13T06:07:08", nil},
	...
	for i, c := range cs {
		d, err := ParseInLocation(c.s, time.UTC)
		if err != nil {
			if c.e == nil {
				t.Fatal(err)
			}
			if err.Error() != c.e.Error() {
				t.Fatalf("[%d] ParseInLocation(%q) = %q, want %q", i, c.s, err.Error(), c.e.Error())
			}
			continue
		}
		...
		if a != c.w {
			t.Errorf("[%d] ParseInLocation(%q) = %q, want %q", i, c.s, a, c.w)
		}
	}
}
...
func TestParse(t *testing.T) {
	...
	for i, c := range cs {
		...
		if err != nil {
			...
			if err.Error() != c.e.Error() {
				t.Fatalf("[%d] Parse(%q) = %q, want %q", i, c.s, err.Error(), c.e.Error())
			}
			continue
		}
